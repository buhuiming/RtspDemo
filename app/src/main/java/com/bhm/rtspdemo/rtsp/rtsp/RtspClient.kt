...
import com.bhm.rtspdemo.rtsp.rtsp.commands.CommandsManager
import com.bhm.rtspdemo.rtsp.rtsp.commands.Method
import com.bhm.rtspdemo.rtsp.utils.ConnectCheckerRtsp
import com.bhm.rtspdemo.rtsp.utils.CreateSSLSocket.createSSlSocket
        ...
        try {
          ...
          when (announceResponse.status) {
            403 -> {
              ...
              Timber.e("Response 403, access denied")
              return@post
            }
            401 -> {
  ...
  private fun disconnect(clear: Boolean) {
    ...
    executor.execute post@{
      try {
        ...
        Timber.i("write teardown success")
      } catch (e: IOException) {
        if (clear) {
          commandsManager.clear()
        } else {
