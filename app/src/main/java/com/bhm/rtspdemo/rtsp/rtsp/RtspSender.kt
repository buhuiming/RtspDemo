...
import com.bhm.rtspdemo.rtsp.rtcp.BaseSenderReport
import com.bhm.rtspdemo.rtsp.rtp.packets.*
import com.bhm.rtspdemo.rtsp.rtp.sockets.BaseRtpSocket
import com.bhm.rtspdemo.rtsp.rtp.sockets.RtpSocketTcp
import com.bhm.rtspdemo.rtsp.utils.BitrateManager
...
open class RtspSender(private val connectCheckerRtsp: ConnectCheckerRtsp) : VideoPacketCallback,
  ...
  fun setVideoInfo(sps: ByteArray, pps: ByteArray, vps: ByteArray?) {
    videoPacket = if (vps == null) H264Packet(sps, pps, this) else H265Packet(sps, pps, this)
  }
  ...
  override fun onVideoFrameCreated(rtpFrame: RtpFrame) {
    try {
      ...
      Timber.i("Video frame discarded")
      droppedVideoFrames++
    }
  }
  ...
  override fun onAudioFrameCreated(rtpFrame: RtpFrame) {
    try {
      ...
      Timber.i("Audio frame discarded")
      droppedAudioFrames++
    }
  }
    ...
    thread?.execute post@{
      ...
      while (!Thread.interrupted() && running) {
        try {
          ...
          if (rtpFrame == null) {
            Timber.i("Skipping iteration, frame null")
            continue
          }
